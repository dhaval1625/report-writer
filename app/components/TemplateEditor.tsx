...
export default function TemplateEditor({ templates, onChange }: TemplateEditorProps) {
  ...
  const updateCurrentTemplate = (updates: Partial<ReportTemplate>) => {
    ...
    onChange(
      templates.map((t) => (t.id === selectedTemplateId ? { ...t, ...updates } : t))
    );
  };
    ...
    <div className="flex flex-col gap-6 rounded-2xl border border-card-border bg-card-bg p-6 shadow-sm transition-all duration-300">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-card-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted">Configure the layout & placeholders for your reports</p>
        </div>
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  <input
                    ...
                    onChange={(e) => {
                      e.stopPropagation();
                      onChange(
                        templates.map((x) => (x.id === t.id ? { ...x, isActive: e.target.checked } : x))
                      );
                    }}
        ...
        {currentTemplate ? (
          <div className="md:col-span-2 flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-1.5">Template Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-1.5">Default Bullet Symbol</label>
                <input
                  type="text"
                  value={currentTemplate.bulletPrefix}
                  onChange={(e) => updateCurrentTemplate({ bulletPrefix: e.target.value })}
                  placeholder="e.g. ➤, -, •"
                  className="w-full rounded-lg border border-card-border bg-background px-3 py-2 text-sm outline-none focus:border-accent"
                />
              </div>
            ...
            <div className="rounded-lg bg-muted-bg p-3.5 border border-card-border/50 text-xs flex flex-col gap-1.5">
              <span className="font-semibold text-foreground/90">Available placeholders:</span>
              <ul className="list-disc pl-4 space-y-1 text-muted">
                <li><code className="text-accent bg-accent/5 px-1 rounded">{"{{points}}"}</code> — Injects points with default bullet prefix.</li>
                <li><code className="text-accent bg-accent/5 px-1 rounded">{"{{points:➤}}"}</code> — Injects points with custom bullet prefix (<code className="font-mono">➤</code>).</li>
                <li><code className="text-accent bg-accent/5 px-1 rounded">{"{{date:DD MMM YYYY}}"}</code> — Renders current date (e.g. <code className="font-mono">18 JUN 2026</code>).</li>
                <li><code className="text-accent bg-accent/5 px-1 rounded">{"{{info:Label}}"}</code> — Dynamically prompts you to input &quot;Label&quot; (e.g., Ticket ID, PM Name).</li>
              </ul>
            </div>
          </div>
        ) : (
